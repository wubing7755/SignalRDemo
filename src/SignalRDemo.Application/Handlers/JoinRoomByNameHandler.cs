...
/// 按房间名称加入房间处理器
/// </summary>
public class JoinRoomByNameHandler : IRequestHandler<JoinRoomByNameCommand, Result<RoomDto>>
{
    private readonly IRoomRepository _roomRepository;

    public JoinRoomByNameHandler(IRoomRepository roomRepository)
    {
        _roomRepository = roomRepository;
    }

    public async Task<Result<RoomDto>> Handle(JoinRoomByNameCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var room = await _roomRepository.GetByNameAsync(roomName, cancellationToken);
            if (room == null)
            {
                return Result<RoomDto>.Failure($"未找到名为 '{request.RoomName}' 的房间", "ROOM_NOT_FOUND");
            }
    ...
    private static RoomDto MapToDto(Domain.Aggregates.ChatRoom room) => new()
    {
        Id = room.Id.Value,
